...
def get_worker_nodes_internal_ips():
    try:
        # Configurazione del cliente Kind
        ...
        # nodi worker
        worker_nodes = [node for node in nodes.items if any(label for label in node.metadata.labels.keys() if label.endswith("/worker"))]
        print(worker_nodes)

        # indirizzi IP interni dei nodi worker
        internal_ips = []
...
def startServer():
    ...
    while True:
        ...
        # Dividi la richiesta in comando e valore
        ...
        if command == "getSuggestion":
            ...
            # Converti il dizionario in una stringa JSON e invia
            response_json = json.dumps(response)
            client_socket.send(response_json.encode("utf-8"))

        # Chiudi la connessione
        client_socket.close()
        print("Suggestion sent to client.")

        # get_nodes_network_usage(["172.19.0.4", "172.19.0.3", "172.19.0.5"])
