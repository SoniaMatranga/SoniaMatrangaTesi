...
class SchedulingEnv(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        self.max_state = np.full((node_count()-1), float('inf'), dtype=np.float32)
        ...
        self.screen = None
        self.policy="LA" #set MA, LA , LC or P0 policy
    ...
    def step(self, action):
        ...
        node_names = get_worker_nodes_internal_ips()
        action_node_ip = 0
        ...
        if self.policy == "MA":
            ...
            if node_to_select != -1:        
                terminated = action.__ne__(node_to_select)  
            else:
                print("No node suggested. Suggest don't schedule.")

            # REWARD   
            unused_nodes = sum(1 for value in self.state if value < 5)
            reward = int( 0 if terminated else (unused_nodes/(self.num_of_nodes -1))*10 ) 
            self.state=get_nodes_state(self.policy, self.worker_ips) 
        ...
        ########################   LatencyAware Most Allocated  POLICY  ############################
        ##################################################################################
            ...
            for i, latency in enumerate(self.state[-3:]):  #find node with lowest latency
        ...
        if self.actual_step < self.test_steps:
            self.actual_step +=1
            if action.__eq__(node_to_select):
                self.success_count +=1
        else:
            self.actual_step = 0 
            success_rate = self.success_count / self.test_steps 
            success_rate = min(success_rate, 1)
            file_path = '/etc/kubernetes/venv/lib/python3.9/site-packages/gymnasium/envs/classic_control/success_rate.txt'
            with open(file_path, 'a') as f:
                current_time = datetime.now().strftime("%H:%M:%S.%f")[:-3] 
                f.write(str(current_time)  + ' ' + str(success_rate)  +'\n')
    ...
    def graphs(self):
        ...
        app.layout = html.Div(
            style={'backgroundColor': '#222222', 'color': 'white', 'height': '200vh','display': 'flex', 'flexDirection': 'column', 'alignItems': 'center', 'margin': '0'},
            children=[
                html.H1(children='Monitoring Dashboard', style={'textAlign': 'center'}), 
                html.Div([  
                    dcc.Graph(id='node-pods'),
        ...
        #Success rate graph
        initial_figure_3 = go.Figure()
        initial_figure_3.add_trace(go.Scatter(x=[], y=[], mode='lines', name='Success Rate'))
        initial_figure_3.update_layout(title='Success rate Chart', xaxis_title='Steps', yaxis_title='Success Rate')
        ...
        def update_graph_2(n_intervals):
            ...
            file_path = '/etc/kubernetes/venv/lib/python3.9/site-packages/gymnasium/envs/classic_control/latency.txt'
        ...
        def update_graph_3(n_intervals):
            ...
            with open(file_path, 'r') as f:
                for line in f:
                    ...
                    x_data.append(time)
                    y_data.append(float(rate))

            x_data = list(x_data) 
            y_data = list(y_data)  

            
            trace1 = go.Scatter(x=x_data, y=y_data, mode='lines', marker={'size': 5}, name='Success Rate')
            
            updated_figure = {'data': [trace1], 'layout': {
                'title': 'Success Rate Chart',
                'xaxis': {'title': 'Steps', 'autorange': 'true', 'tickmode': 'array', 'tickvals':[], 'ticktext': []},
    ...
    def env_method(self, method_name, *args, **kwargs):
        if method_name == "action_masks":
            ...
            raise NotImplementedError(f"Method {method_name} not supported on this environment.")
    ...
    def close(self):
        ...
        file_path4 = '/etc/kubernetes/venv/lib/python3.9/site-packages/gymnasium/envs/classic_control/success_rate.txt'
